import { CPack, CPackUnit, CPackUnitModel, Cw4Map, Model } from "./cw4.js";
import { produceHighlighted } from "../highlighter.js";
import { addTab, selectTopTab } from "../mapEditor.js";
import { Vector4 } from "../memory.js";
...
async function genMapMesh(data: Cw4Map) {
    ...
    function pushCol(count: number, col?: Vector4) {
        if (!col) {
            color.push(1, 1, 1, 1);
            return;
        }
    ...
    for (let x = 0; x < width - 1; x++) {
        for (let y = 0; y < height - 1; y++) {
            ...
            if (tl == tr && tl == bl) {
                ...
                uv.push(
                    x1, tr * wallScale, wallTexture,
                    x2, br * wallScale, wallTexture,
                    x2, bl * wallScale, wallTexture,
                );
                pushCol(3, wallColor);
                continue;
            }
            ...
            if (tl == tr || tr == br || br == bl || bl == tl) {
                ...
                if (tl == tr || bl == br) {
                    uv.push(
                        x1, tl * wallScale, wallTexture, // tl
                        x2, tr * wallScale, wallTexture, // tr
                        x2, br * wallScale, wallTexture, // br

                        x1, tl * wallScale, wallTexture, // tl
                        x2, br * wallScale, wallTexture, // br
                        x1, bl * wallScale, wallTexture, // bl
                    );
                } else {
                    uv.push(
                        x2, tl * wallScale, wallTexture, // tl
                        x2, tr * wallScale, wallTexture, // tr
                        x1, br * wallScale, wallTexture, // br
            ...
            if (tr == bl) {
                ...
                uv.push(
                    x2, tl * wallScale, wallTexture,
                    x1, tr * wallScale, wallTexture,
                    x2, bl * wallScale, wallTexture,

                    x1, tr * wallScale, wallTexture,
                    x1, br * wallScale, wallTexture,
                    x2, bl * wallScale, wallTexture,
                );
                pushCol(6, wallColor);
                continue;
        ...
        uv.push(
            x1, 0, wallTexture,
            x2, getH(x + 1, 0) * wallScale, wallTexture,
    ...
    for (let y = 0; y < height - 1; y++) {
        ...
        uv.push(
            x1, getH(0, y) * wallScale, wallTexture,
            x2, getH(0, y + 1) * wallScale, wallTexture,
            x1, 0, wallTexture,
            ...
            x1, 0, wallTexture,
            x2, getH(width - 1, y + 1) * wallScale, wallTexture,
            x1, getH(width - 1, y) * wallScale, wallTexture,
...
export function genUnitMesh(model: Model): THREE.BufferGeometry {
    // buffer mesh
    // @ts-ignore
    if (model.Mesh) return model.Mesh;
...
async function createObj(pack: CPack, cpackUnit: CPackUnitModel) {
    let tex: THREE.Texture = THREE.Texture.DEFAULT_IMAGE;
    ...
    if (cpackUnit.Model) {
        let mat = new THREE.MeshBasicMaterial({
            ...
            color: new THREE.Color(
                cpackUnit.Color.x * cpackUnit.ColorBrightness,
                cpackUnit.Color.y * cpackUnit.ColorBrightness,
                cpackUnit.Color.z * cpackUnit.ColorBrightness
                // asdf.Color.w
            )
        });

        if (obj = await getModel(cpackUnit.Model)) {
            let m = (obj.children[0] as THREE.Mesh);
            mat.vertexColors = (m.material as THREE.Material).vertexColors;
            m.material = mat;
        } else {
...
export class cw4MapViewer {
    ...
    async loadMap(data: Cw4Map) {
        ...
        let units = new Map<string, [CPack, CPackUnit]>();
        if (data.CPacks) {
        ...
        for (const unit of data.Units) {
            ...
            if (temp) {
                obj = temp.obj;
                if (temp.mixer) this.mixers.push(temp.mixer);
            } else {
...
class CPackWindow {
    ...
    packSelect: HTMLSelectElement;

    unitsEl: ListThing;
    meshesEl: ListThing;
    texturesEl: ListThing;
    scriptsEl: ListThing;
    ...
    update() {
        ...
        pack.Units?.forEach(x =>
            this.unitsEl.add(x.Name, () => {
        ...
        pack.Models?.forEach(x =>
            this.meshesEl.add(x.Name, () => {
                let m = new MeshViewer(`${pack.Name} - ${x.Name}`);
                m.addModel(x);
                m.start();
            }));
        pack.Textures?.forEach(x =>
            this.texturesEl.add(x.Name, () => {
        ...
        pack.Scripts?.forEach(x =>
            this.scriptsEl.add(x.Name, () => {
