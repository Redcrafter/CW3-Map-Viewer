...
async function decompress(buffer: ArrayBuffer) {
    ...
    let result = await new Promise<any>(r => LZMA.decompress(arr, r));
...
function compress(game: Game) {
    ...
    return new Promise<number[]>(resolve => LZMA.compress(xmlString, 6, resolve));
}
